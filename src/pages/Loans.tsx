...
export function Loans() {
  ...
  const validate = () => {
    ...
    if (!form.personName.trim()) errs.personName = 'Nhập tên ngư�i';
    if (!form.amount || form.amount <= 0) errs.amount = 'Nhập số ti�n';
    if (!form.walletId) errs.walletId = 'Ch�n ví';
    setErrors(errs);
  ...
  return (
    <div className="px-4 py-5 lg:px-6 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        ...
        <div className="rounded-[1.5rem] p-4 text-white" style={{ background: 'linear-gradient(135deg,#10b981,#059669)', boxShadow: '0 8px 24px rgba(16,185,129,0.3)' }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-sm opacity-80 font-medium">Ngư�i nợ tôi</p>
          </div>
              ...
              <div key={loan.id} className="bg-white rounded-[1.5rem] p-4 shadow-sm border border-gray-50">
                ...
                <div className="mb-3">
                  ...
                  <div className="flex justify-between mt-1">
                    <p className="text-[10px] text-[#877275]">�ã trả {progress.toFixed(0)}%</p>
                    {loan.dueDate && (
                      <p className={`text-[10px] font-semibold ${overdue ? 'text-[#9b3f5a]' : days !== null && days <= 7 ? 'text-amber-500' : 'text-[#877275]'}`}>
                        {overdue ? `Quá hạn ${Math.abs(days!)} ngày ⚠�` : days !== null ? `Còn ${days} ngày` : ''}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {loan.status === 'paid' && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-[#146a5f] font-semibold bg-[#a4f1e3]/20 px-3 py-1 rounded-xl">✅ �ã hoàn thành</span>
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title={form.type === 'borrow' ? '💸 Thêm khoản vay' : '� Thêm khoản cho vay'}>
        <div className="p-5 space-y-4">
          ...
          <Input
            ...
            placeholder="Tên ngư�i..."
            error={errors.personName}
          />

          <div>
            <label className="block text-sm font-medium text-[#544245] mb-1.5">Số ti�n</label>
            <input
              type="text"
              inputMode="numeric"
      ...
      {/* Pay Modal */}
      <Modal isOpen={showPayModal} onClose={() => setShowPayModal(false)} title={payTarget?.type === 'borrow' ? '💳 Trả ti�n' : '💵 Nhận ti�n'} size="sm">
        <div className="p-5 space-y-4">
          {payTarget && (
          ...
          <Input label="Ghi chú" value={payNote} onChange={e => setPayNote(e.target.value)} placeholder="Tùy ch�n..." />
