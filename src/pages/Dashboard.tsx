...
function MobileSpendingSummary({
  dailyData,
  topCategoriesCount,
  savingsRate,
}: {
  ...
  const maxValue = Math.max(
    1,
    ...recentDays.map(day => Number(day.income || 0)),
    ...recentDays.map(day => Number(day.expense || 0)),
  );
    ...
    <div className="bg-white rounded-[1.5rem] p-5 shadow-pink-md border border-[#ffd9e0]/20">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-3 text-[10px] text-[#877275]">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm inline-block bg-[#89d4c7]" />Thu</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm inline-block bg-[#ff8fab]" />Chi</span>
        </div>
      </div>

      <div className="flex h-32 items-end gap-1">
        {recentDays.map(day => {
          const incomeHeight = Math.max(4, (Number(day.income || 0) / maxValue) * 100);
          const expenseHeight = Math.max(4, (Number(day.expense || 0) / maxValue) * 100);
          return (
            <div key={String(day.day)} className="flex-1 flex items-end justify-center gap-0.5">
              <div className="w-full max-w-[6px] rounded-t bg-[#89d4c7]" style={{ height: `${incomeHeight}%` }} />
              <div className="w-full max-w-[6px] rounded-t bg-[#ff8fab]" style={{ height: `${expenseHeight}%` }} />
            </div>
          );
        })}
      </div>

      <div className="pt-3 border-t border-[#f8f2f4] flex justify-between text-[10px] text-[#877275] mt-4">
        <span>Tổng danh mục: {topCategoriesCount}</span>
        <span>{savingsRate >= 20 ? 'Chi tiêu lành mạnh' : 'Theo dõi chi tiêu'}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="pb-4 max-w-xl mx-auto lg:max-w-5xl space-y-4">
      ...
      <div className="px-4 space-y-4">
        ...
        {isMobile ? (
          <>
            <MobileSpendingSummary dailyData={dailyData} topCategoriesCount={topCategories.length} savingsRate={insights.savingsRate} />
            {topCategories.length > 0 && (
              <MobileCategorySummary topCategories={topCategories} onOpenReports={() => navigate('/reports')} />
            )}
          </>
        ) : (
          <Suspense fallback={<div className="bg-white rounded-[1.5rem] p-5 border border-[#ffd9e0]/20 h-56" />}>
            <DashboardCharts
              dailyData={dailyData}
