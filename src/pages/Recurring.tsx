...
export function Recurring() {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) errs.name = 'Nhập tên khoản';
  ...
  const expenseItems = recurring.filter(r => (r.type || 'expense') === 'expense');
      ...
      <Card key={item.id} className={!item.isActive ? 'opacity-50' : ''}>
        <div className="flex items-start gap-3">
          <div
            ...
            style={{ background: (cat?.color || (isIncome ? '#146a5f' : '#9b3f5a')) + '20' }}
          >
            {cat?.icon || (isIncome ? '💰' : '💸')}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <p className={`font-bold text-sm flex-shrink-0 ${isIncome ? 'text-[#146a5f]' : 'text-[#9b3f5a]'}`}>
                {isIncome ? '+' : '-'}{formatCurrency(item.amount)}
              </p>
            </div>

            {isDueSoon && !alreadyCreated && (
              <div className="mt-2 text-xs text-amber-600 font-medium">⏰ Sắp đến hạn!</div>
            )}
  ...
  return (
    <div className="px-4 py-5 lg:px-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-bold text-[#1d1b1d]">Thu chi cố định</h1>
      ...
      {recurring.length === 0 ? (
        <EmptyState
          icon="🔄"
          title="Chưa có khoản cố định nào"
          description="Thêm các khoản thu chi định kỳ như tiền lương, tiền nhà, internet..."
        ...
        <div className="space-y-5">
          {incomeItems.length > 0 && (
            <div>
              <p className="text-xs font-bold text-[#146a5f] uppercase tracking-wide mb-2">Thu nhập định kỳ</p>
              <div className="space-y-3">{incomeItems.map(renderItem)}</div>
            </div>
          )}
          {expenseItems.length > 0 && (
            <div>
              <p className="text-xs font-bold text-[#9b3f5a] uppercase tracking-wide mb-2">Chi tiêu cố định</p>
              <div className="space-y-3">{expenseItems.map(renderItem)}</div>
            </div>
          )}
        </div>
      )}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editTarget ? 'Sửa khoản cố định' : 'Thêm khoản cố định'}>
        <div className="p-5 space-y-4">
          {/* Type toggle */}
          <div>
            <p className="text-sm font-medium text-[#544245] mb-2">Loại</p>
            <div className="flex gap-2">
              <button
                onClick={() => setForm(f => ({ ...f, type: 'expense', categoryId: expenseCategories[0]?.id || '' }))}
                className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-colors ${form.type === 'expense' ? 'bg-[#9b3f5a] text-white' : 'bg-[#f8f2f4] text-[#877275]'}`}
              >
                💸 Chi tiêu
              </button>
              <button
                onClick={() => setForm(f => ({ ...f, type: 'income', categoryId: incomeCategories[0]?.id || '' }))}
                className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-colors ${form.type === 'income' ? 'bg-[#146a5f] text-white' : 'bg-[#f8f2f4] text-[#877275]'}`}
              >
                💰 Thu nhập
              </button>
            </div>
          ...
          <Input
            ...
            placeholder={form.type === 'income' ? 'VD: Lương tháng, thưởng...' : 'VD: Tiền nhà, Netflix...'}
            error={errors.name}
          />
