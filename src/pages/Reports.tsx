...
export function Reports() {
  ...
  return (
    <div className="px-4 py-5 lg:px-6 max-w-4xl mx-auto space-y-5">
      <h1 className="text-xl font-bold text-[#1d1b1d]">Báo cáo</h1>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-[#a4f1e3]/20 rounded-[1.25rem] p-4">
          <p className="text-xs text-[#146a5f] font-medium mb-1">Tổng thu</p>
        ...
        <div className="bg-[#ffd9e0]/20 rounded-[1.25rem] p-4">
          <p className="text-xs text-[#9b3f5a] font-medium mb-1">Tổng chi</p>
        ...
        <div className={`${selectedMonthData.savings >= 0 ? 'bg-[#ffd9e0]/30' : 'bg-orange-50'} rounded-[1.25rem] p-4`}>
          <p className={`text-xs font-medium mb-1 ${selectedMonthData.savings >= 0 ? 'text-[#9b3f5a]' : 'text-[#f97316]'}`}>Tiết kiệm</p>
          ...
          <p className="text-[10px] text-[#877275]">{selectedMonthData.savingsRate.toFixed(0)}% tỷ lệ TK</p>
        </div>
      </div>
      ...
      <Card padding={false}>
        <div className="p-4 pb-0">
          <h3 className="font-semibold text-sm text-[#1d1b1d]">Xu hướng 6 tháng</h3>
        </div>
        ...
        <div className="flex gap-4 px-4 pb-3">
          <div className="flex items-center gap-1.5 text-xs text-[#877275]"><div className="w-2.5 h-2.5 rounded-sm bg-[#22c55e]" /> Thu</div>
      ...
      <Card padding={false}>
        <div className="p-4 pb-0">
          <h3 className="font-semibold text-sm text-[#1d1b1d]">Tiết kiệm hàng tháng</h3>
        </div>
      ...
      {selectedMonthData.categoryData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card padding={false}>
            <div className="p-4 pb-0">
              <h3 className="font-semibold text-sm text-[#1d1b1d]">Chi tiêu theo danh mục</h3>
            </div>
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
            ...
            {prevMonth ? (
              <>
                <div className="flex items-center justify-between mb-3 p-3 rounded-xl bg-[#f8f2f4]">
                  <span className="text-xs text-[#877275]">Tổng chi tháng này</span>
                ...
                <div className="space-y-2">
                  <p className="text-xs text-[#877275] font-medium">Danh mục tăng mạnh nhất:</p>
                  ...
                  {categoryComparison.filter(c => c.change > 0).length === 0 && (
                    <p className="text-xs text-[#146a5f]">🎉 Chi tiêu giảm so với tháng trước!</p>
                  )}
                </div>
              </>
            ) : (
              <p className="text-sm text-[#877275] text-center py-4">Chưa có dữ liệu tháng trước</p>
            )}
          </Card>
        </div>
      )}
